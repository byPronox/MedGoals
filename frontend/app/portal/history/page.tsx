import { getEmployees, getMedGoalsEmployeeDetail, getPerformanceLogs, getEmployeeDetail } from '@/lib/odoo';
import EmployeeSelector from './EmployeeSelector';
import { ScoreEvolutionChart, LogsActivityChart } from './Charts';
import './history.css';

export default async function ActivityHistoryPage({ searchParams }: { searchParams: Promise<{ employee_id?: string }> }) {
        ...
        <div className="analytics-grid">
             ...
             <div className="summary-flex">
                <div className="summary-avatar">
                    {empDetail?.avatar_128 ? (
                         <img src={`data:image/png;base64,${empDetail.avatar_128}`} alt="Avatar" />
                    ) : (
                ...
                <div className="summary-info">
                    ...
                    <div className="badges-row">
                        <span className="badge-area">{selectedEmployeeData?.med_area?.name || 'N/A'}</span>
                        {empDetail?.is_top_performer && <span className="badge-top">⭐ Top Performer</span>}
                    </div>
                </div>
                <div className="summary-kpi">
                    <small>Último Score</small>
                    <strong>{selectedEmployeeData?.last_score?.toFixed(1) || '0.0'}</strong>
                </div>
             </div>
          </div>
          ...
          <div className="card chart-card">
            <div className="card-header">
                ...
                <span className="subtitle">Histórico de puntajes totales por ciclo</span>
            </div>
          ...
          <div className="card activity-card">
             <div className="card-header">
                ...
                <span className="subtitle">Logs generados en los últimos días</span>
             </div>
          ...
          <div className="card logs-table-card full-width">
            <div className="card-header">
                ...
                <span className="count-badge">{performanceLogs.length} eventos</span>
            </div>
                    ...
                    <tbody>
                        {performanceLogs.map(log => (
                            <tr key={log.id}>
                                ...
                                <td>
                                    {log.metric_value ? (
                                        <span className="metric-tag">{log.metric_value}</span>
                                    ) : (
                                ...
                                <td>
                                    {log.assignment ? (
                                        <span className="assignment-link">🔗 {log.assignment.name}</span>
                                    ) : (
                                        <span className="text-muted">-</span>
                                    )}
                                </td>
                         ...
                         {performanceLogs.length === 0 && (
                            <tr>
                                <td colSpan={4} className="text-center p-4">No hay logs de actividad registrados.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
